                  ...
                  int16_t *y, int n, int16_t alpha)
{
    ...
    for (; n > 0; n -= vl) {
        ...
        // Widening Multiply (i16 -> i32)
 
        vint32m8_t vprod = __riscv_vwmul_vx_i32m8(vb, alpha, vl);

        // Widening accumulation (i32 + i32 -> i32)
       
        vint32m8_t vsum = __riscv_vwadd_wv_i32m8(vprod, va, vl);

        // Narrowing operaton  (i32 -> i16 )
        vint16m4_t vy = __riscv_vnclip_wx_i16m4(vsum, 0, vl);
